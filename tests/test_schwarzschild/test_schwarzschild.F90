! deprecated tests... B_eff now returns not the effective planck function but the source term itself
!@test(npes=[1])
...
!    @assertEqual(Blev(k + 1), B/pi, .1_ireals, 'for very high optical depth, the values should be approx the same')
!    @assertTrue(is_between(B/pi, Blev(k+1), Blev(k)), 'resulting B_eff has to be between the far and near value'//toStr([Blev(k+1), B, Blev(k)])//' (tau='//toStr(tau)//')')
!  end do
!
!  tau = 0._ireals
!  do k = 1, Nlay
!    call B_eff(Blev(k), Blev(k + 1), tau, B)
!    !print *,k,Blev(k), Blev(k+1), B, is_between(B,Blev(k), Blev(k+1))
!    @assertEqual((Blev(k) + Blev(k + 1)) / 2, B/pi, .1_ireals, 'for zero optical depth, should be approx the mean')
!    @assertTrue(is_between(B/pi, Blev(k+1), Blev(k)), 'resulting B_eff has to be between the far and near value'//toStr([Blev(k+1), B, Blev(k)])//' (tau='//toStr(tau)//')')
!  end do
!
!  do itau = -100, 40
