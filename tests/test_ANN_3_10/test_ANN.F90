...
contains
  ...
  subroutine test_compare_diff2diff_to_LUT(this)
    ...
    associate (Ndiff => OPP_LUT%LUT%diff_streams)
      ...
      do isrc = 1, Ndiff
        print *, 'C_LUT src', isrc, ':', trim(colored_str_by_range(pC_LUT(isrc, :), limits=climits, colors=colors)), &
          & sum(pC_LUT(isrc, :))
      end do
      print *, ''
      ...
      do isrc = 1, Ndiff
        print *, 'C_ANN src', isrc, ':', trim(colored_str_by_range(pC_ANN(isrc, :), limits=climits, colors=colors)), &
          & sum(pC_ANN(isrc, :))
      end do
      print *, ''
