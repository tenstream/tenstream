...
contains
  ...
  subroutine init_vegetation_types_simple(ierr)
    ...
    veg(2)%lambda = [real(ireals) :: &
      & 0.205,0.403,0.499,0.515,0.527,0.543,0.563,0.591,0.679,0.688,0.694,0.708,0.714,0.72,0.736,0.746,0.754,0.76,0.775,0.807,&
      & 0.914,0.933,0.957,0.976,1.068,1.104,1.128,1.144,1.154,1.179,1.204,1.258,1.299,1.318,1.353,1.374,1.398,1.404,1.408,1.423,&
      & 1.448,1.474,1.592,1.634,1.676,1.716,1.772,1.835,1.855,1.865,1.885,1.895,1.915,1.935,2.015,2.125,2.215,2.265,2.466,2.656,&
      & 2.688,2.752,2.784,2.816,2.944,2.976 ] * 1e-6_ireals
    veg(2)%albedo = [real(ireals) :: &
      & 0.02,0.027,0.041,0.06,0.085,0.095,0.089,0.064,0.039,0.049,0.072,0.172,0.224,0.287,0.485,0.588,0.637,0.66,0.686,0.7,&
      & 0.704,0.695,0.663,0.659,0.699,0.693,0.671,0.619,0.593,0.574,0.572,0.594,0.575,0.537,0.452,0.395,0.224,0.197,0.181,0.154,&
      & 0.142,0.153,0.304,0.336,0.344,0.327,0.285,0.277,0.242,0.205,0.1,0.066,0.043,0.042,0.079,0.144,0.168,0.155,0.054,0.029,&
      & 0.013,0.003,0.02,0.0,0.012,0.003 ]

    veg(3)%vname = "leaf" ! from usgs Aspen_Leaf-A DW92-2           BECKa AREF
    veg(3)%lambda = [real(ireals) :: &
      & 0.353,0.499,0.519,0.531,0.553,0.597,0.688,0.694,0.7,0.708,0.728,0.734,0.74,0.751,0.769,0.851,0.949,1.084,1.198,1.244,&
      & 1.303,1.333,1.378,1.384,1.408,1.423,1.448,1.468,1.534,1.592,1.647,1.7,1.772,1.835,1.855,1.865,1.895,1.905,1.945,2.155,&
      & 2.235,2.285,2.496,2.56,2.592 ] * 1e-6_ireals
    veg(3)%albedo = [real(ireals) :: &
      & 0.032,0.039,0.053,0.076,0.087,0.053,0.037,0.045,0.073,0.135,0.332,0.379,0.411,0.442,0.458,0.46,0.444,0.438,0.398,0.403,&
      & 0.398,0.375,0.322,0.305,0.181,0.141,0.124,0.126,0.199,0.247,0.268,0.263,0.233,0.233,0.217,0.192,0.058,0.038,0.029,0.111,&
