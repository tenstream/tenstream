...
#include "petsc/finclude/petsc.h"
  ...
  use m_plex_grid, only: t_plexgrid, compute_face_geometry, &
    ...
    determine_incoming_outgoing_offsets, &
  ...
  use m_pprts_base, only : t_solver_log_events, setup_log_events
  ...
  use m_plex_rt_base, only: &
    ...
    t_plex_solver_18_8, &
    t_field, &
    t_state_container_plexrt, &
    prepare_solution, destroy_solution, &
    setup_field_IS
...
!  use m_plexrt_external_solvers, only: plexrt_schwarz, plexrt_twostream, plexrt_disort, plexrt_NCA_wrapper
  ...
  integer(iintegers), parameter :: &
    FIELD_EDIR_COLLAPSE=0, &
    FIELD_EDIR_1D      =1, &
    FIELD_EDIR_3D_TOP  =2, &
    FIELD_EDIR_3D_SIDE =3, &
    FIELD_EDIF_COLLAPSE=4, &
    FIELD_EDIF_1D      =5, &
    FIELD_EDIF_3D_TOP  =6, &
    FIELD_EDIF_3D_SIDE =7
  ...
  contains
    subroutine init_plex_rt_solver(plex, solver)
      ...
      class is (t_plex_solver_5_8)
        ...
        call setup_default_fields_info(&
          & dirtopdof   = i1, dirtop_area_divider  = i1, &
          & dirsidedof  = i1, dirside_area_divider = i1, &
          & difftopdof  = i2, difftop_area_divider = i1, &
          & diffsidedof = i4, diffside_area_divider= i4, &
          & fieldsinfo = solver%fields)

      class is (t_plex_solver_rectilinear_5_8)
        allocate(t_optprop_rectilinear_wedge_5_8::solver%OPP)
        call setup_default_fields_info(i1, i1, i1, i1, i2, i1, i4, i1, solver%fields)

      class is (t_plex_solver_18_8)
        allocate(t_optprop_wedge_18_8::solver%OPP)
        call setup_default_fields_info(i3, i3, i4, i4, i2, i1, i4, i1, solver%fields)
...
!        & dm = solver%plex%ediff_dm)
      ...
      call setup_mergedm(solver%fields, solver%mergedm)

      contains
        subroutine setup_default_fields_info(&
            dirtopdof, dirtop_area_divider, &
            dirsidedof, dirside_area_divider, &
            difftopdof, difftop_area_divider, &
            diffsidedof, diffside_area_divider, &
            fieldsinfo )
          integer(iintegers), intent(in) :: dirtopdof, dirtop_area_divider
          integer(iintegers), intent(in) :: dirsidedof, dirside_area_divider
          integer(iintegers), intent(in) :: difftopdof, difftop_area_divider
          integer(iintegers), intent(in) :: diffsidedof, diffside_area_divider
          type(t_field), allocatable, intent(inout) :: fieldsinfo(:)

          allocate(fieldsinfo(0:7))
          fieldsinfo(FIELD_EDIR_COLLAPSE)%fieldname = 'edir_collapse'
          fieldsinfo(FIELD_EDIR_COLLAPSE)%fieldname = 'edir_collapse'
          fieldsinfo(FIELD_EDIR_1D      )%fieldname = 'edir_1D'
          fieldsinfo(FIELD_EDIR_3D_TOP  )%fieldname = 'edir_3D_top'
          fieldsinfo(FIELD_EDIR_3D_SIDE )%fieldname = 'edir_3D_side'
          fieldsinfo(FIELD_EDIF_COLLAPSE)%fieldname = 'ediff_collapse'
          fieldsinfo(FIELD_EDIF_1D      )%fieldname = 'ediff_1D'
          fieldsinfo(FIELD_EDIF_3D_TOP  )%fieldname = 'ediff_3D_top'
          fieldsinfo(FIELD_EDIF_3D_SIDE )%fieldname = 'ediff_3D_side'

          fieldsinfo(FIELD_EDIR_COLLAPSE)%dof = i1
          fieldsinfo(FIELD_EDIR_1D      )%dof = i1
          fieldsinfo(FIELD_EDIR_3D_TOP  )%dof = dirtopdof
          fieldsinfo(FIELD_EDIR_3D_SIDE )%dof = dirsidedof
          fieldsinfo(FIELD_EDIF_COLLAPSE)%dof = i2
          fieldsinfo(FIELD_EDIF_1D      )%dof = i2
          fieldsinfo(FIELD_EDIF_3D_TOP  )%dof = difftopdof
          fieldsinfo(FIELD_EDIF_3D_SIDE )%dof = diffsidedof

          fieldsinfo(FIELD_EDIR_COLLAPSE)%streams = i1
          fieldsinfo(FIELD_EDIR_1D      )%streams = i1
          fieldsinfo(FIELD_EDIR_3D_TOP  )%streams = dirtopdof
          fieldsinfo(FIELD_EDIR_3D_SIDE )%streams = dirsidedof
          fieldsinfo(FIELD_EDIF_COLLAPSE)%streams = i2
          fieldsinfo(FIELD_EDIF_1D      )%streams = i2
          fieldsinfo(FIELD_EDIF_3D_TOP  )%streams = difftopdof/i2
          fieldsinfo(FIELD_EDIF_3D_SIDE )%streams = diffsidedof/i2

          fieldsinfo(FIELD_EDIR_COLLAPSE)%area_divider = i1
          fieldsinfo(FIELD_EDIR_1D      )%area_divider = i1
          fieldsinfo(FIELD_EDIR_3D_TOP  )%area_divider = dirtop_area_divider
          fieldsinfo(FIELD_EDIR_3D_SIDE )%area_divider = dirside_area_divider
          fieldsinfo(FIELD_EDIF_COLLAPSE)%area_divider = i1
          fieldsinfo(FIELD_EDIF_1D      )%area_divider = i1
          fieldsinfo(FIELD_EDIF_3D_TOP  )%area_divider = difftop_area_divider
          fieldsinfo(FIELD_EDIF_3D_SIDE )%area_divider = diffside_area_divider
        end subroutine
        subroutine setup_mergedm(fieldsinfo, dm)
          type(t_field), intent(inout) :: fieldsinfo(0:)
          type(tDM), allocatable, intent(inout) :: dm
          integer(iintegers) :: fStart, fEnd, ifield
          integer(iintegers) :: cStart, cEnd

          integer(iintegers) :: numfields
          integer(iintegers) :: cdof(0:size(fieldsinfo)-1)
          integer(iintegers) :: edof(0:size(fieldsinfo)-1)
          integer(iintegers) :: vdof(0:size(fieldsinfo)-1)
          logical, allocatable :: fmasks(:,:), useCone(:), useClosure(:)
          type(tVec) :: gVec
          type(tPetscSection) :: mergeSection
          ...
          if(allocated(dm)) call CHKERR(1_mpiint, 'called setup_mergedm on an already allocated DM')
          allocate(dm)
          call DMClone(solver%plex%dm, dm, ierr); call CHKERR(ierr)
          call DMSetOptionsPrefix(dm, 'mergedm_', ierr); call CHKERR(ierr)

          call DMPlexGetDepthStratum(dm, i2, fStart, fEnd, ierr); call CHKERR(ierr) ! faces
          call DMPlexGetDepthStratum(dm, i3, cStart, cEnd, ierr); call CHKERR(ierr) ! faces
          allocate(&
            fmasks    (0:numfields-1, fStart:fEnd-1), &
            useCone   (0:numfields-1), &
            useClosure(0:numfields-1)  )

          fmasks(:,:) = .False.
          fmasks(FIELD_EDIR_COLLAPSE,:) = solver%plex%ltopfacepos(fStart:fEnd-1)                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).eq.1                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).eq.Ncollapse+1

          call setup_field_IS(cStart, cEnd,                   &
            solver%plex%zindex(cStart:cEnd-1).ge.1.and.       &
            solver%plex%zindex(cStart:cEnd-1).le.Ncollapse,   &
            fieldsinfo(FIELD_EDIR_COLLAPSE)%cellIS)

          call setup_field_IS(fStart, fEnd,                   &
            solver%plex%zindex(fStart:fEnd-1).eq.1,           &
            fieldsinfo(FIELD_EDIR_COLLAPSE)%topfaceIS)

          call setup_field_IS(fStart, fEnd,                   &
            solver%plex%zindex(fStart:fEnd-1).eq.Ncollapse+1, &
            fieldsinfo(FIELD_EDIR_COLLAPSE)%botfaceIS)

          fmasks(FIELD_EDIR_1D      ,:) = solver%plex%ltopfacepos(fStart:fEnd-1)                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).ge.Ncollapse+1                .and.&
                                          solver%plex%zindex(fStart:fEnd-1).le.plex%max_constrained_k+1

          call setup_field_IS(cStart, cEnd,                               &
            solver%plex%zindex(cStart:cEnd-1).ge.Ncollapse+1.and.         &
            solver%plex%zindex(cStart:cEnd-1).le.plex%max_constrained_k,  &
            fieldsinfo(FIELD_EDIR_1D)%cellIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.Ncollapse+1,              &
            fieldsinfo(FIELD_EDIR_1D)%topfaceIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.plex%max_constrained_k+1, &
            fieldsinfo(FIELD_EDIR_1D)%botfaceIS)


          fmasks(FIELD_EDIR_3D_TOP  ,:) = solver%plex%ltopfacepos(fStart:fEnd-1)                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).ge.plex%max_constrained_k+1   .and.&
                                          solver%plex%zindex(fStart:fEnd-1).le.plex%Nlay+1

          call setup_field_IS(cStart, cEnd,                                     &
            solver%plex%zindex(cStart:cEnd-1).ge.plex%max_constrained_k+1.and.  &
            solver%plex%zindex(cStart:cEnd-1).le.plex%Nlay,                     &
            fieldsinfo(FIELD_EDIR_3D_TOP)%cellIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.plex%max_constrained_k+1, &
            fieldsinfo(FIELD_EDIR_3D_TOP)%topfaceIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.plex%Nlay+1,              &
            fieldsinfo(FIELD_EDIR_3D_TOP)%botfaceIS)


          fmasks(FIELD_EDIR_3D_SIDE ,:) = .not.solver%plex%ltopfacepos(fStart:fEnd-1)                     .and.&
                                          solver%plex%zindex(fStart:fEnd-1).ge.plex%max_constrained_k+1   .and.&
                                          solver%plex%zindex(fStart:fEnd-1).le.plex%Nlay

          call setup_field_IS(cStart, cEnd,                                     &
            solver%plex%zindex(cStart:cEnd-1).ge.plex%max_constrained_k+1.and.  &
            solver%plex%zindex(cStart:cEnd-1).le.plex%Nlay,                     &
            fieldsinfo(FIELD_EDIR_3D_SIDE)%cellIS)


          ! Diffuse Radiation
          fmasks(FIELD_EDIF_COLLAPSE,:) = solver%plex%ltopfacepos(fStart:fEnd-1)                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).eq.1                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).eq.Ncollapse+1
          ...
          call setup_field_IS(cStart, cEnd,                   &
            solver%plex%zindex(cStart:cEnd-1).ge.1.and.       &
            solver%plex%zindex(cStart:cEnd-1).le.Ncollapse,   &
            fieldsinfo(FIELD_EDIF_COLLAPSE)%cellIS)

          call setup_field_IS(fStart, fEnd,                   &
            solver%plex%zindex(fStart:fEnd-1).eq.1,           &
            fieldsinfo(FIELD_EDIF_COLLAPSE)%topfaceIS)

          call setup_field_IS(fStart, fEnd,                   &
            solver%plex%zindex(fStart:fEnd-1).eq.Ncollapse+1, &
            fieldsinfo(FIELD_EDIF_COLLAPSE)%botfaceIS)

          fmasks(FIELD_EDIF_1D      ,:) = solver%plex%ltopfacepos(fStart:fEnd-1)                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).ge.Ncollapse+1                .and.&
                                          solver%plex%zindex(fStart:fEnd-1).le.plex%max_constrained_k+1
          ...
          call setup_field_IS(cStart, cEnd,                               &
            solver%plex%zindex(cStart:cEnd-1).ge.Ncollapse+1.and.         &
            solver%plex%zindex(cStart:cEnd-1).le.plex%max_constrained_k,  &
            fieldsinfo(FIELD_EDIF_1D)%cellIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.Ncollapse+1,              &
            fieldsinfo(FIELD_EDIF_1D)%topfaceIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.plex%max_constrained_k+1, &
            fieldsinfo(FIELD_EDIF_1D)%botfaceIS)

          fmasks(FIELD_EDIF_3D_TOP  ,:) = solver%plex%ltopfacepos(fStart:fEnd-1)                          .and.&
                                          solver%plex%zindex(fStart:fEnd-1).ge.plex%max_constrained_k+1   .and.&
                                          solver%plex%zindex(fStart:fEnd-1).le.plex%Nlay+1
          ...
          call setup_field_IS(cStart, cEnd,                                     &
            solver%plex%zindex(cStart:cEnd-1).ge.plex%max_constrained_k+1.and.  &
            solver%plex%zindex(cStart:cEnd-1).le.plex%Nlay,                     &
            fieldsinfo(FIELD_EDIF_3D_TOP)%cellIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.plex%max_constrained_k+1, &
            fieldsinfo(FIELD_EDIF_3D_TOP)%topfaceIS)

          call setup_field_IS(fStart, fEnd,                                &
            solver%plex%zindex(fStart:fEnd-1).eq.plex%Nlay+1,              &
            fieldsinfo(FIELD_EDIF_3D_TOP)%botfaceIS)


          fmasks(FIELD_EDIF_3D_SIDE ,:) = .not.solver%plex%ltopfacepos(fStart:fEnd-1)                     .and.&
                                          solver%plex%zindex(fStart:fEnd-1).ge.plex%max_constrained_k+1   .and.&
                                          solver%plex%zindex(fStart:fEnd-1).le.plex%Nlay
          ...
          call setup_field_IS(cStart, cEnd,                                     &
            solver%plex%zindex(cStart:cEnd-1).ge.plex%max_constrained_k+1.and.  &
            solver%plex%zindex(cStart:cEnd-1).le.plex%Nlay,                     &
            fieldsinfo(FIELD_EDIF_3D_SIDE)%cellIS)


          ! Set adjacency to be adjacent if q is in closure(star(dof)), i.e. if face is connected via a cell
          useCone = .False.
          useClosure = .True.

          call dmplex_set_new_section(dm, &
            & 'flux_section', &
            & numfields, &
            & cdof, fieldsinfo(:)%dof, edof, vdof, &
            & fieldsinfo(:)%fieldname, &
            & fmasks=fmasks, &
            & useCone=useCone, useClosure=useClosure, &
            & point_major=.True.)
          call DMGetSection(dm, mergeSection, ierr); call CHKERR(ierr)

          do ifield = 0, numfields-1
            fieldsinfo(ifield)%id = ifield
            allocate(fieldsinfo(ifield)%subIS)
            allocate(fieldsinfo(ifield)%subdm)
            call DMCreateSubDM(dm, i1, [ifield], fieldsinfo(ifield)%subIS, fieldsinfo(ifield)%subdm, ierr); call CHKERR(ierr)
            call PetscObjectSetName(fieldsinfo(ifield)%subdm, trim(fieldsinfo(ifield)%fieldname), ierr); call CHKERR(ierr)
            call setup_IS_in_out_dof( &
              fieldsinfo(ifield)%subdm, &
              solver%plex%ownerlabel, &
              fieldsinfo(ifield)%cellIS, &
              fieldsinfo(ifield)%streams, &
              mergeSection, &
              ifield, &
              fieldsinfo(ifield)%dofdm, &
              fieldsinfo(ifield)%in_out_dof)
          enddo

          call DMGetGlobalVector(dm, gVec, ierr); call CHKERR(ierr)
          call PetscObjectViewFromOptions(gVec, dm, &
            '-show_debug_flx', ierr); call CHKERR(ierr)
        ...
        subroutine plexrt_view_geometry(comm)
          ...
          associate( plex => solver%plex, geom_dm => solver%plex%geom_dm )
            ...
            call DMGetStratumIS(plex%dm, 'DomainBoundary', TOAFACE, boundary_ids, ierr); call CHKERR(ierr)
    ...
    subroutine destroy_plexrt_solver(solver, lfinalizepetsc)
      ...
      if(allocated(solver)) then
        ...
        call dealloc_vec(solver%scalevec_Wm2_to_W)
        call dealloc_vec(solver%scalevec_W_to_Wm2)
...
!    subroutine get_in_out_dof_offsets(IS_in_out_dof, icell, incoming_offsets, outgoing_offsets, opt_xv)
!      type(tIS), intent(in) :: IS_in_out_dof
...
!      else
!        call ISGetIndicesF90(IS_in_out_dof, xv, ierr); call CHKERR(ierr)
!        call with_is_vec(xv)
!        call ISRestoreIndicesF90(IS_in_out_dof, xv, ierr); call CHKERR(ierr)
!      endif
!    contains
!      subroutine with_is_vec(xis)
!        integer(iintegers), intent(in), pointer :: xis(:)
!        integer(iintegers) :: bs, idx_offset_in, idx_offset_out
!
!        bs = xis(size(xis))
...
!
!        idx_offset_in = icell*i2*bs
!        idx_offset_out= icell*i2*bs+bs
!        incoming_offsets(:) = xis(i1+idx_offset_in :idx_offset_in +bs)
!        outgoing_offsets(:) = xis(i1+idx_offset_out:idx_offset_out+bs)
!      end subroutine
!    end subroutine

    subroutine setup_IS_in_out_dof(dm, ownerlabel, cellIS, Nstreams, section, ifield, dof_dm, in_out_dof)
      type(tDM), allocatable, intent(in) :: dm
      type(tDMLabel), intent(in) :: ownerlabel
      type(tIS), intent(in) :: cellIS
      integer(iintegers), intent(in) :: Nstreams, ifield
      type(tPetscSection), intent(in) :: section
      type(tDM), allocatable, intent(inout) :: dof_dm
      type(tVec), allocatable, intent(inout) :: in_out_dof
      real(ireals), pointer :: xin_out_dof(:)

      integer(iintegers) :: i, icell, bs
      integer(iintegers), allocatable :: incoming_offsets(:), outgoing_offsets(:)
      integer(mpiint) :: comm, myid, ierr

      integer(iintegers),pointer :: faces_of_cell(:), cellidx(:)
      integer(iintegers) :: ndof, num_dof, max_num_dof, iface, inc_off, out_off
      character(len=default_str_len) :: prefix

      type(tPetscSection) :: dof_section
      integer(iintegers) :: cStart, cEnd
      logical, allocatable :: cmasks(:,:)

      if(allocated(in_out_dof)) call CHKERR(1_mpiint, 'IS in_out_dof already allocated')
      if(allocated(dof_dm)) call CHKERR(1_mpiint, 'dof_dm has already allocated')
      if(.not.allocated(dm)) call CHKERR(1_mpiint, 'dm has to be allocated')

      call PetscObjectGetComm(dm, comm, ierr); call CHKERR(ierr)

      call ISGetIndicesF90(cellIS, cellidx, ierr); call CHKERR(ierr)

      call DMPlexGetDepthStratum(dm, i3, cStart, cEnd, ierr); call CHKERR(ierr) ! cells
      allocate(cmasks(i2, cStart:cEnd-i1), source=.False.)
      ...
      do i = 1, size(cellidx)
        ...
        do iface = 1, size(faces_of_cell)
          call PetscSectionGetFieldDof(section, faces_of_cell(iface), ifield, ndof, ierr); call CHKERR(ierr)
      ...
      allocate(dof_dm)
      call DMClone(dm, dof_dm, ierr); call CHKERR(ierr)
      call dmplex_set_new_section(dof_dm, &
        & 'inout_dof_section', &
        & i2, &
        & [max_num_dof, max_num_dof], [i0,i0], [i0,i0], [i0,i0], &
        & ['inc_dofs', 'out_dofs'], cmasks=cmasks)
      call DMGetSection(dof_dm, dof_section, ierr); call CHKERR(ierr)

      if(ldebug) then
        call mpi_comm_rank(comm, myid, ierr); call CHKERR(ierr)
        call PetscObjectGetName(dm, prefix, ierr); call CHKERR(ierr)
        if(myid.eq.0) print *,'( '//trim(prefix)//' ) IS_in_out_dof blocksize = '//itoa(bs)
      endif

      allocate(in_out_dof)
      call DMCreateGlobalVector(dof_dm, in_out_dof, ierr); call CHKERR(ierr)

      call VecGetArrayF90(in_out_dof, xin_out_dof, ierr); call CHKERR(ierr)

      do i = 1, size(cellidx)
        icell = cellidx(i)
        call determine_incoming_outgoing_offsets(dm, ownerlabel, Nstreams, section, &
          ifield, icell, &
          & incoming_offsets, outgoing_offsets)

        call PetscSectionGetFieldOffset(dof_section, icell, i0, inc_off, ierr); call CHKERR(ierr)
        call PetscSectionGetFieldOffset(dof_section, icell, i1, out_off, ierr); call CHKERR(ierr)

        xin_out_dof(i1+inc_off:inc_off+bs) = incoming_offsets(:)
        xin_out_dof(i1+out_off:out_off+bs) = outgoing_offsets(:)

        print *,bs, 'icell', icell, 'incoming ', itoa(incoming_offsets), ' out ', itoa(outgoing_offsets), &
                    'xinout',itoa(int(xin_out_dof(i1+inc_off:out_off+bs)))
      enddo

      call VecRestoreArrayF90(in_out_dof, xin_out_dof, ierr); call CHKERR(ierr)
      call ISRestoreIndicesF90(cellIS, cellidx, ierr); call CHKERR(ierr)
    end subroutine
    ...
    subroutine run_plex_rt_solver(solver, lthermal, lsolar, sundir, opt_solution_uid, opt_solution_time)
      ...
      if(.not.solver%solutions(suid)%lset) then
        call prepare_solution(solver%mergedm, solver%plex%abso_dm, &
          solution=solver%solutions(suid), uid=suid)
      endif
      ...
      associate( solution => solver%solutions(suid) )
        ...
        call create_edir_src_vec(solver, solver%plex, solver%mergedm, norm2(sundir), &
          solver%kabs, solver%ksca, solver%g, &
          sundir/norm2(sundir), solver%dirsrc)
        call PetscObjectSetName(solver%dirsrc, 'srcvec', ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(solver%dirsrc, PETSC_NULL_VEC, &
          '-show_src_vec', ierr); call CHKERR(ierr)
        call PetscLogEventEnd(solver%logs%setup_dir_src, ierr)

        ! Output of srcVec
        if(ldebug) then
          call scale_flx(solver, solver%plex, &
            solver%scalevec_Wm2_to_W, solver%scalevec_W_to_Wm2, &
            solution, lWm2=.False., logevent=solver%logs%scale_flx)
          call debug_dump_vec(solver%mergedm, solver%dirsrc, solver%scalevec_W_to_Wm2)
        endif

        ! Create Direct Matrix
        call PetscLogEventBegin(solver%logs%setup_Mdir, ierr)
        call create_mat(solver, solver%plex, solver%OPP, solver%kabs, solver%ksca, solver%g, &
        ...
        call solve_plex_rt(solver%mergedm, &
          solver%dirsrc, &
          solver%Mdir, &
          solver%ksp_solar_dir, &
          solution%flx, &
          ksp_residual_history=solution%ksp_residual_history, &
          prefix='mergedm_', &
          ksp_iter=solution%Niter)
        call PetscLogEventEnd(solver%logs%solve_Mdir, ierr)
        call PetscObjectSetName(solution%flx, 'flx', ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(solution%flx, PETSC_NULL_VEC, &
          '-show_flx_vec', ierr); call CHKERR(ierr)
        solution%lWm2 = .False.
        ...
        if(ldebug) then
          call scale_flx(solver, solver%plex, &
            solver%scalevec_Wm2_to_W, solver%scalevec_W_to_Wm2, &
            solution, lWm2=.False., logevent=solver%logs%scale_flx)
          call debug_dump_vec(solver%mergedm, solution%flx, solver%scalevec_W_to_Wm2)
        endif
...
!          call debug_dump_vec(solver%plex%ediff_dm, solution%ediff, solver%diff_scalevec_W_to_Wm2)
      ...
      contains
        subroutine check_input_arguments()
          ...
          if(.not.allocated(solver%mergedm))  &
            call CHKERR(1_mpiint, 'solver%mergedm not allocated, should have happened in init_solver?')
    ...
    subroutine create_edir_src_vec(solver, plex, edirdm, E0, kabs, ksca, g, sundir, srcVec)
      ...
      if (boundary_ids.eq.PETSC_NULL_IS) then ! dont have TOA boundary faces
        ...
        do i = 1, size(xx_v)
          ...
          area = geoms(i1+geom_offset) / real(solver%fields(FIELD_EDIR_COLLAPSE)%area_divider, ireals)
          ...
          if(is_solar_src(face_normal, sundir)) then
            ...
            call PetscSectionGetFieldOffset(edirsection, iface, FIELD_EDIR_COLLAPSE, voff, ierr); call CHKERR(ierr)
            call PetscSectionGetFieldDof(edirSection, iface, FIELD_EDIR_COLLAPSE, numDof, ierr); call CHKERR(ierr)
            do idof = 1, numDof
      ...
      contains
        subroutine set_sidewards_direct_fluxes()
          ...
          type(tPetscSection) :: horizsection
          type(tVec) :: vedir
          real(ireals), pointer :: xedir(:)

          call DMGetGlobalVector(plex%horizface1_dm, vedir, ierr); call CHKERR(ierr)
          ...
          if (IS_side_faces.eq.PETSC_NULL_IS) then ! dont have side boundary faces
            ...
            call DMGetSection(plex%horizface1_dm, horizsection, ierr); call CHKERR(ierr)
            ...
            call VecSet(vedir, -9999._ireals, ierr); call CHKERR(ierr)
            ...
            do o = 1, size(iside_faces)
              ...
              call PetscSectionGetOffset(horizsection, topface, topoffset, ierr); call CHKERR(ierr)
              ...
              do k = 1, plex%Nlay
                ...
                call PetscSectionGetOffset(horizsection, topface, topoffset, ierr); call CHKERR(ierr)
                call PetscSectionGetOffset(horizsection, botface, botoffset, ierr); call CHKERR(ierr)
            ...
            do o = 1, size(iside_faces)
              iface_side = iside_faces(o)
              call PetscSectionGetFieldDof(edirSection, iface_side, FIELD_EDIR_3D_SIDE, numDof, ierr); call CHKERR(ierr)
              ...
              area = geoms(i1+geom_offset) / real(solver%fields(FIELD_EDIR_3D_SIDE)%area_divider, ireals)
              ...
              call PetscSectionGetFieldOffset(edirsection, iface_side, FIELD_EDIR_3D_SIDE, voff, ierr); call CHKERR(ierr)
              do idof=1,numDof
...
!      end subroutine
    ...
    subroutine scale_facevec(plex, mergedm, fieldsinfo, globalfaceVec, lW_to_Wm2)
    type(t_plexgrid), intent(in) :: plex
    type(tDM), intent(in) :: mergedm
    type(t_field), intent(in) :: fieldsinfo(:)
    ...
    integer(iintegers) :: numfields, ifield
    ...
    call DMGetSection(mergedm, faceSection, ierr); call CHKERR(ierr)

    call PetscSectionGetNumFields(faceSection, numfields, ierr); call CHKERR(ierr)

    do ifield=0, numfields-1
      call scale_field(ifield, fieldsinfo(i1+ifield)%area_divider)
    enddo

    contains
      subroutine scale_field(ifield, area_divider)
        integer(iintegers), intent(in) :: ifield, area_divider
        ...
        integer(iintegers) :: fStart, fEnd, geom_offset, face_offset
        integer(iintegers) :: iface, idof, num_dof

        call DMGetSection(plex%geom_dm, geomSection, ierr); CHKERRQ(ierr)
        call VecGetArrayReadF90(plex%geomVec, geoms, ierr); call CHKERR(ierr)

        call VecGetArrayF90(globalfaceVec, xv, ierr); call CHKERR(ierr)

        call DMPlexGetDepthStratum(mergedm, i2, fStart, fEnd, ierr); call CHKERR(ierr)

        do iface = fStart, fEnd-1
          call PetscSectionGetFieldOffset(geomSection, iface, i2, geom_offset, ierr); call CHKERR(ierr)
          area = geoms(i1+geom_offset) / real(area_divider, ireals)

          call PetscSectionGetFieldOffset(faceSection, iface, ifield, face_offset, ierr); call CHKERR(ierr)
          call PetscSectionGetFieldDof(faceSection, iface, ifield, num_dof, ierr); call CHKERR(ierr)
          if(lW_to_Wm2) then
            do idof = 1, num_dof
        ...
        call VecRestoreArrayF90(globalfaceVec, xv, ierr); call CHKERR(ierr)
        call VecRestoreArrayReadF90(plex%geomVec, geoms, ierr); call CHKERR(ierr)
      end subroutine
    end subroutine

  subroutine create_mat(solver, plex, OPP, kabs, ksca, g, sundir, A)
    ...
    type(tPetscSection) :: cell1Section, geomSection, matSection, srfc_boundarySection
    real(ireals), pointer :: xkabs(:), xksca(:), xg(:)
    real(ireals), pointer :: xgeoms(:)

    call pre()

    call set1DMat(solver%fields(FIELD_EDIR_COLLAPSE), solver%fields(FIELD_EDIF_COLLAPSE))
    call set1DMat(solver%fields(FIELD_EDIR_1D)      , solver%fields(FIELD_EDIF_1D))
    call set1DMat(solver%fields(FIELD_EDIR_3D_TOP)  , solver%fields(FIELD_EDIF_3D_TOP))

    call set_albedo( &
      solver%fields(FIELD_EDIR_3D_TOP), &
      solver%fields(FIELD_EDIR_3D_TOP)%botfaceIS, &
      solver%fields(FIELD_EDIF_3D_TOP), &
      solver%fields(FIELD_EDIF_3D_TOP)%botfaceIS, &
      solver%albedo )

    call couple_fields( &
      solver%fields(FIELD_EDIR_COLLAPSE), solver%fields(FIELD_EDIR_COLLAPSE)%botfaceIS, i0, &
      solver%fields(FIELD_EDIR_1D)      , solver%fields(FIELD_EDIR_1D)%topfaceIS      , i0  )
    call couple_fields( &
      solver%fields(FIELD_EDIR_1D)      , solver%fields(FIELD_EDIR_1D)%botfaceIS      , i0, &
      solver%fields(FIELD_EDIR_3D_TOP)  , solver%fields(FIELD_EDIR_3D_TOP)%topfaceIS  , i0  )

    ! Couple src: edn@collapse => dst: edn@1D
    call couple_fields( &
      solver%fields(FIELD_EDIF_COLLAPSE), solver%fields(FIELD_EDIF_COLLAPSE)%botfaceIS, i0, &
      solver%fields(FIELD_EDIF_1D)      , solver%fields(FIELD_EDIF_1D)%topfaceIS      , i0  )
    ! Couple src: eup@1D => dst: eup@collapse
    call couple_fields( &
      solver%fields(FIELD_EDIF_1D)      , solver%fields(FIELD_EDIF_1D)%topfaceIS      , i1, &
      solver%fields(FIELD_EDIF_COLLAPSE), solver%fields(FIELD_EDIF_COLLAPSE)%botfaceIS, i1  )

    ! Couple src: edn@1D => dst: edn@3D
    call couple_fields( &
      solver%fields(FIELD_EDIF_1D)      , solver%fields(FIELD_EDIF_1D)%botfaceIS      , i0, &
      solver%fields(FIELD_EDIF_3D_TOP)  , solver%fields(FIELD_EDIF_3D_TOP)%topfaceIS  , i0  )
    ! Couple src: eup@3D => dst: eup@1D
    call couple_fields( &
      solver%fields(FIELD_EDIF_3D_TOP)  , solver%fields(FIELD_EDIF_3D_TOP)%topfaceIS  , i1, &
      solver%fields(FIELD_EDIF_1D)      , solver%fields(FIELD_EDIF_1D)%botfaceIS      , i1  )

    print *, 'DEBUG uninit', OPP%OPP_LUT%dirconfig%dims(1)%vrange, sundir

    call finish()
    contains

      subroutine set_albedo(src_field, src_field_is, dst_field, dst_field_is, albedo)
        type(t_field), intent(in) :: src_field, dst_field
        type(tIS), intent(in) :: src_field_is, dst_field_is
        type(tVec), intent(in) :: albedo

        integer(iintegers), pointer :: srcidx(:), dstidx(:)
        integer(iintegers) :: i, srcface, dstface, offset_srfc
        integer(iintegers) :: numdof_src, numdof_dst, num_incoming_diffuse_dof
        integer(iintegers) :: icol, irow, isrc, idst
        real(ireals), pointer :: xalbedo(:)
        real(ireals) :: Ag

        !DEBUG integer(iintegers), pointer :: cell_support(:)
        !DEBUG integer(iintegers), pointer :: xinoutdof(:)
        !DEBUG integer(iintegers), allocatable :: incoming_offsets(:), outgoing_offsets(:)
        !DEBUG call DMPlexGetSupport(dst_field%subdm, dstface, cell_support, ierr); call CHKERR(ierr)
        !DEBUG icell = cell_support(1)
        !DEBUG call DMPlexRestoreSupport(dst_field%subdm, dstface, cell_support, ierr); call CHKERR(ierr)
        !DEBUG call get_in_out_dof_offsets(dst_field%IS_in_out_dof, icell, incoming_offsets, outgoing_offsets, xinoutdof)

        call ISGetIndicesF90(src_field_is, srcidx, ierr); call CHKERR(ierr)
        call ISGetIndicesF90(dst_field_is, dstidx, ierr); call CHKERR(ierr)

        call CHKERR(int(size(srcidx)-size(dstidx),mpiint), &
          'both coupling IS`s need to have the same size but found:'// &
          'src '//itoa(size(srcidx))// &
          'dst '//itoa(size(dstidx))   )
        ...
        do i=1,size(srcidx)
          ...
          call PetscSectionGetFieldDof(matSection, srcface, src_field%id, numdof_src, ierr); call CHKERR(ierr)
          call PetscSectionGetFieldOffset(matSection, srcface, src_field%id, icol, ierr); call CHKERR(ierr)

          call PetscSectionGetFieldDof(matSection, dstface, dst_field%id, numdof_dst, ierr); call CHKERR(ierr)
          call PetscSectionGetFieldOffset(matSection, dstface, dst_field%id, irow, ierr); call CHKERR(ierr)

          num_incoming_diffuse_dof = numdof_dst/2

          call PetscSectionGetOffset(srfc_boundarySection, dstface, offset_srfc, ierr); call CHKERR(ierr)
          Ag = xalbedo(i1+offset_srfc)

          ! sum up all direct radiation dof and put them equally on the diffuse outgoing dof
          do isrc = 0, numdof_src-1
            do idst = 0, numdof_dst-1, 2 ! at the ground, first one is incoming, then every second one
              call MatSetValuesLocal(A, i1, irow+isrc, i1, icol+idst, -Ag, INSERT_VALUES, ierr); call CHKERR(ierr)
            enddo
          enddo
      ...
      subroutine couple_fields(src_field, src_field_is, src_dof, dst_field, dst_field_is, dst_dof)
        type(t_field), intent(in) :: src_field, dst_field
        type(tIS), intent(in) :: src_field_is, dst_field_is
        integer(iintegers), intent(in) :: src_dof, dst_dof

        integer(iintegers), pointer :: srcidx(:), dstidx(:)
        integer(iintegers) :: i, srcface, dstface
        integer(iintegers) :: numdof_src, numdof_dst
        ...
        do i=1,size(srcidx)
          ...
          call CHKERR(int(numdof_src-numdof_dst,mpiint), 'not yet implemented for numdof_src != numdof_dst')

          print *,'Coupling '//trim(src_field%fieldname)//' <--> '//trim(dst_field%fieldname)// &
            & ' row '//itoa(irow+dst_dof)// &
            & ' col '//itoa(icol+src_dof)
          call MatSetValuesLocal(A, i1, irow+dst_dof, i1, icol+src_dof, -one, INSERT_VALUES, ierr); call CHKERR(ierr)

        enddo

        call ISRestoreIndicesF90(src_field_is, srcidx, ierr); call CHKERR(ierr)
        call ISRestoreIndicesF90(dst_field_is, dstidx, ierr); call CHKERR(ierr)
      end subroutine

      subroutine set1DMat(dir_field, diff_field)
        use iso_c_binding, only: c_loc
        type(t_field), intent(in) :: dir_field, diff_field
        integer(iintegers), pointer :: idx(:), faces_of_cell(:)
        integer(iintegers) :: cell1_offset, geom_offset
        real(ireals) :: dtau, w0, g, dz, mu0
        real(ireals) :: area_top, area_bot, Ar(2), face_normal(3)
        real(ireals) :: c11,c12,c13,c23,c33

        integer(iintegers) :: fStart, fEnd

        integer(iintegers) :: i, icell, iface
        integer(iintegers) :: numdof
        integer(iintegers) :: topface, botface
        integer(iintegers) :: irow, icol
        integer(iintegers) :: subdm_diff_offset
        integer(iintegers) :: irows(2), icols(3)
        real(ireals) :: coeffs(6)

        type(tPetscSection) :: dof_section
        real(ireals), pointer :: xinoutdof(:)
        integer(iintegers) :: inc_off, out_off, bs
        integer(iintegers), allocatable :: incoming_offsets(:), outgoing_offsets(:)

        print *,'Setting Collapse Mat for ifield '//itoa(dir_field%id)
        call PetscObjectViewFromOptions(dir_field%subdm, solver%mergedm, &
          '-show_mat_subdm', ierr); call CHKERR(ierr)

        call DMPlexGetDepthStratum(dir_field%subdm, i2, fStart, fEnd, ierr); call CHKERR(ierr)
        do iface=fStart, fEnd-1
          call PetscSectionGetFieldDof(matSection, iface, dir_field%id, numdof, ierr); call CHKERR(ierr)
          call PetscSectionGetFieldOffset(matSection, iface, dir_field%id, irow, ierr); call CHKERR(ierr)
          do icol=0,numdof-1
            call MatSetValuesLocal(A, i1, irow+icol, i1, irow+icol, one, INSERT_VALUES, ierr); call CHKERR(ierr)
          enddo
        enddo
        call DMPlexGetDepthStratum(diff_field%subdm, i2, fStart, fEnd, ierr); call CHKERR(ierr)
        do iface=fStart, fEnd-1
          call PetscSectionGetFieldDof(matSection, iface, diff_field%id, numdof, ierr); call CHKERR(ierr)
          call PetscSectionGetFieldOffset(matSection, iface, diff_field%id, irow, ierr); call CHKERR(ierr)
          do icol=0,numdof-1
            call MatSetValuesLocal(A, i1, irow+icol, i1, irow+icol, one, INSERT_VALUES, ierr); call CHKERR(ierr)
          enddo
        enddo

        if(.not.allocated(dir_field%cellIS)) call CHKERR(1_mpiint, 'dir_field%cellIS not allocated')
        call ISGetIndicesF90(dir_field%cellIS, idx, ierr); call CHKERR(ierr)

        call ISGetLocalSize(diff_field%cellIS, i, ierr); call CHKERR(ierr)
        call CHKERR(int(size(idx)-i, mpiint), 'dir and diff field cell IS`s dont match in size! Cant be non-conforming!')

        call DMGetSection(diff_field%dofdm, dof_section, ierr); call CHKERR(ierr)
        call VecGetArrayReadF90(diff_field%in_out_dof, xinoutdof, ierr); call CHKERR(ierr)
        call PetscSectionGetFieldComponents(dof_section, i0, bs, ierr); call CHKERR(ierr)
        allocate(incoming_offsets(bs), outgoing_offsets(bs))

        call DMPlexGetDepthStratum(diff_field%subdm, i2, fStart, fEnd, ierr); call CHKERR(ierr)
        call PetscSectionGetFieldOffset(matSection, fStart, diff_field%id, subdm_diff_offset, ierr); call CHKERR(ierr)

        do i=1,size(idx)
          icell = idx(i)

          call DMPlexGetCone(dir_field%subdm, icell, faces_of_cell, ierr); call CHKERR(ierr)
          topface = faces_of_cell(1)
          botface = faces_of_cell(2)
          call DMPlexRestoreCone(dir_field%subdm, icell, faces_of_cell, ierr); call CHKERR(ierr)

          call get_inward_face_normal(topface, icell, geomSection, xgeoms, face_normal)
          mu0 = -dot_product(sundir, -face_normal)

          call PetscSectionGetFieldOffset(geomSection, topface, i2, geom_offset, ierr); call CHKERR(ierr)
          area_top = xgeoms(i1+geom_offset)
          call PetscSectionGetFieldOffset(geomSection, botface, i2, geom_offset, ierr); call CHKERR(ierr)
          area_bot = xgeoms(i1+geom_offset)

          call PetscSectionGetFieldOffset(geomSection, icell, i3, geom_offset, ierr); call CHKERR(ierr)
          dz = xgeoms(i1+geom_offset)

          call PetscSectionGetOffset(cell1Section, icell, cell1_offset, ierr); call CHKERR(ierr)
          dtau = (xkabs(i1+cell1_offset) + xksca(i1+cell1_offset) ) * dz
          w0   = xksca(i1+cell1_offset) / (xkabs(i1+cell1_offset) + xksca(i1+cell1_offset))
          call eddington_coeff_zdun(dtau, w0, g, mu0, c11, c12, c13, c23, c33)
          !DEBUG:
          !c11=.1
          !c12=0
          !c13=0
          ...
          coeffs = [c13, c12, c11, c23, c11, c12]
          ! super compensate direct radiation !TODO check
          Ar(1) = area_top / area_bot
          Ar(2) = area_bot / area_top
          ! TODO: coeffs = coeffs * [one, one, Ar(1), Ar(2), Ar(2), one]

          ! Direct radiation:
          call PetscSectionGetFieldOffset(matSection, botface, dir_field%id, irow, ierr); call CHKERR(ierr)
          call PetscSectionGetFieldOffset(matSection, topface, dir_field%id, icol, ierr); call CHKERR(ierr)
          call MatSetValuesLocal(A, i1, irow, i1, icol, -c33 * Ar(1), INSERT_VALUES, ierr); call CHKERR(ierr)

          !call get_in_out_dof_offsets(diff_field%IS_in_out_dof, icell, incoming_offsets, outgoing_offsets, xinoutdof)
          call PetscSectionGetFieldOffset(dof_section, icell, i0, inc_off, ierr); call CHKERR(ierr)
          call PetscSectionGetFieldOffset(dof_section, icell, i1, out_off, ierr); call CHKERR(ierr)

          incoming_offsets(:) = int(xinoutdof(i1+inc_off:inc_off+bs), iintegers)
          outgoing_offsets(:) = int(xinoutdof(i1+out_off:out_off+bs), iintegers)
          print *,'icell', icell, 'inc', incoming_offsets, 'out', outgoing_offsets

          ! DST: eup top face
           call PetscSectionGetFieldOffset(matSection, topface, diff_field%id, irows(1), ierr); call CHKERR(ierr)
           irows(1) = irows(1) + 1
          !irows(1) = outgoing_offsets(1) ! DST: eup top face

          ! DST: edn bot face
          call PetscSectionGetFieldOffset(matSection, botface, diff_field%id, irows(2), ierr); call CHKERR(ierr)
          !irows(2) = outgoing_offsets(2) ! DST: edn bot face

          ! SRC: edir top face
          call PetscSectionGetFieldOffset(matSection, topface, dir_field%id, icols(1), ierr); call CHKERR(ierr)

          ! SRC: edn top face
           call PetscSectionGetFieldOffset(matSection, topface, diff_field%id, icols(2), ierr); call CHKERR(ierr)
          !icols(2) = incoming_offsets(1) ! SRC: edn top face

          ! SRC: eup bot face
           call PetscSectionGetFieldOffset(matSection, botface, diff_field%id, icols(3), ierr); call CHKERR(ierr)
           icols(3) = icols(3) + 1 ! +1 because we take second component of diffuse rad
          ! icols(3) = incoming_offsets(2) ! SRC: eup bot face
          print *,'icell', icell, 'irows', irows, 'icols', icols
          ...
          print *,'icell', icell, 'irows', irows, 'icols', icols
          call MatSetValuesLocal(A, i2, irows, i3, icols, -coeffs, INSERT_VALUES, ierr); call CHKERR(ierr)
          !call CHKERR(1_mpiint, 'DEBUG')
        enddo

        call ISRestoreIndicesF90(dir_field%cellIS, idx, ierr); call CHKERR(ierr)
        call VecRestoreArrayReadF90(diff_field%in_out_dof, xinoutdof, ierr); call CHKERR(ierr)
      end subroutine
      ...
      subroutine pre()
        ...
        call PetscObjectViewFromOptions(solver%mergedm, solver%mergedm, &
          '-show_mat_dm', ierr); call CHKERR(ierr)

        if(.not.allocated(A)) then
          allocate(A)
          if(myid.eq.0.and.ldebug) print *,'Creating Direct Matrix...'
          call DMCreateMatrix(solver%mergedm, A, ierr); call CHKERR(ierr)
          call DMGetGlobalVector(solver%mergedm, diag, ierr)   ; call CHKERR(ierr)
          call VecSet(diag, one, ierr); call CHKERR(ierr)
          call MatDiagonalSet(A, diag, INSERT_VALUES, ierr); call CHKERR(ierr)
          call DMRestoreGlobalVector(solver%mergedm, diag, ierr); call CHKERR(ierr)
        ...
        call DMGetSection(plex%cell1_dm, cell1Section, ierr); call CHKERR(ierr)
        ...
        if(.not.allocated(plex%geom_dm)) call CHKERR(1_mpiint, 'plex%geom_dm has to be allocated')
        call DMGetSection(plex%geom_dm, geomSection, ierr); call CHKERR(ierr)
        call VecGetArrayReadF90(plex%geomVec, xgeoms, ierr); call CHKERR(ierr)

        if(.not.allocated(solver%mergedm)) call CHKERR(1_mpiint, 'mergedm has to be allocated')
        call DMGetSection(solver%mergedm, matSection, ierr); call CHKERR(ierr)

        if(.not.allocated(solver%plex%srfc_boundary_dm)) call CHKERR(1_mpiint, 'solver%plex%srfc_boundary_dm has to be allocated')
        call DMGetSection(solver%plex%srfc_boundary_dm, srfc_boundarySection, ierr); call CHKERR(ierr)
      end subroutine
      ...
      subroutine finish()
        ...
        call PetscObjectViewFromOptions(A, solver%mergedm, &
          '-show_mat', ierr); call CHKERR(ierr)
      end subroutine
...
!    type(t_field), intent(in) :: t_top_dof, t_side_dof
!    integer(iintegers), intent(in) :: face_plex2bmc(:)  ! mapping from faces_of_cell to bmc_faces ordering (dim=5)
...
!  end subroutine
  ...
  subroutine compute_absorption(solver, solution)
    ...
    if(solution%lchanged) then
      if(.not.allocated(solution%flx)) call CHKERR(1_mpiint, 'ediff vec not allocated')
      ...
      if(by_flx_divergence) then
        call compute_flx_absorption(solver%plex, solution%flx, solution%abso)
      else
        call compute_flx_absorption_by_coeff_divergence(solver, solver%plex, solution%flx, solution%abso)
      endif
...
!  end subroutine
  ...
  subroutine compute_flx_absorption(plex, ediff, abso)
    type(t_plexgrid), allocatable, intent(in) :: plex
    ...
    do icell = cStart, cEnd-1
      ...
      cell_abso = zero
      do i = 1, 999! size(incoming_offsets)
  ...
  subroutine compute_flx_absorption_by_coeff_divergence(solver, plex, ediff, abso)
    class(t_plex_solver), intent(in) :: solver
    type(t_plexgrid), allocatable, intent(in) :: plex
    type(tVec), intent(in) :: ediff
    type(tVec), allocatable, intent(inout) :: abso
    ...
    ! i.e. face_plex2bmc(1) gives the wedge boxmc position of first dmplex face
    integer(iintegers) :: face_plex2bmc(5)
    integer(iintegers) :: diff_plex2bmc(8) !solver%diffdof/2) !DEBUG wrong dim

    real(irealLUT) :: coeff(8**2)! DEBUG wrong dim(solver%diffdof/2)**2) ! coefficients for each src=[1..8] and dst[1..8]
    ...
    do icell = cStart, cEnd-1
      ...
      do i = 1, size(incoming_offsets)
        if(incoming_offsets(i).ge.0) then
          associate( diff2diff => coeff(diff_plex2bmc(i):size(coeff):size(diff_plex2bmc)) )
    ...
    subroutine scale_flx(solver, plex, &
      ...
      type(t_plexgrid), intent(in) :: plex
      type(tVec), allocatable, intent(inout) :: scalevec_Wm2_to_W, scalevec_W_to_Wm2
      type(t_state_container_plexrt),intent(inout)  :: solution  !< @param solution container with computed fluxes
      ...
      if(solution%lWm2.neqv.lWm2) then
        if(lWm2) then
          call VecPointwiseMult(solution%flx, solution%flx, scalevec_W_to_Wm2, ierr); call CHKERR(ierr)
        else
          call VecPointwiseMult(solution%flx, solution%flx, scalevec_Wm2_to_W, ierr); call CHKERR(ierr)
        endif
        solution%lWm2 = lWm2
      endif
      if(present(logevent)) then
    ...
    contains
      subroutine gen_scale_vecs()
        if(.not.allocated(scalevec_Wm2_to_W)) then
          allocate(scalevec_Wm2_to_W)
          call VecDuplicate(solution%flx, scalevec_Wm2_to_W, ierr); call CHKERR(ierr)
          call VecSet(scalevec_Wm2_to_W, one, ierr); call CHKERR(ierr)
          call scale_facevec(plex, solver%mergedm, &
            solver%fields, &
            scalevec_Wm2_to_W, lW_to_Wm2=.False.)
        endif
        if(.not.allocated(scalevec_W_to_Wm2)) then
          allocate(scalevec_W_to_Wm2)
          call VecDuplicate(scalevec_Wm2_to_W, scalevec_W_to_Wm2, ierr); call CHKERR(ierr)
          call VecSet(scalevec_W_to_Wm2, one, ierr); call CHKERR(ierr)
          call VecPointwiseDivide( &  ! Computes 1./scalevec_Wm2_to_W
            scalevec_W_to_Wm2, &
            scalevec_W_to_Wm2, &
            scalevec_Wm2_to_W, ierr); call CHKERR(ierr)
        endif
      end subroutine
    end subroutine
    ...
    subroutine plexrt_get_result(solver, redn, reup, rabso, redir, opt_solution_uid)
      ...
      ! Concatenate collapse and 1D domain (without top)
      call ISDifference(solver%fields(FIELD_EDIR_1D)%subIS, solver%fields(FIELD_EDIR_1D)%topfaceIS, istmp, ierr); call CHKERR(ierr)
      !call ISExpand(solver%fields(FIELD_EDIR_COLLAPSE)%subIS, istmp, isedir, ierr); call CHKERR(ierr)

      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_COLLAPSE)%subIS, PETSC_NULL_IS, &
        & '-show_c_is', ierr); call CHKERR(ierr)
      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_COLLAPSE)%topfaceIS, PETSC_NULL_IS, &
        & '-show_c_is_top', ierr); call CHKERR(ierr)
      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_COLLAPSE)%botfaceIS, PETSC_NULL_IS, &
        & '-show_c_is_bot', ierr); call CHKERR(ierr)

      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_1D)%subIS, PETSC_NULL_IS, &
        & '-show_1D_is', ierr); call CHKERR(ierr)
      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_1D)%topfaceIS, PETSC_NULL_IS, &
        & '-show_1D_is_top', ierr); call CHKERR(ierr)
      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_1D)%botfaceIS, PETSC_NULL_IS, &
        & '-show_1D_is_bot', ierr); call CHKERR(ierr)

      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_3D_TOP)%subIS, PETSC_NULL_IS, &
        & '-show_3D_is', ierr); call CHKERR(ierr)
      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_3D_TOP)%topfaceIS, PETSC_NULL_IS, &
        & '-show_3D_is_top', ierr); call CHKERR(ierr)
      call PetscObjectViewFromOptions(solver%fields(FIELD_EDIR_3D_TOP)%botfaceIS, PETSC_NULL_IS, &
        & '-show_3D_is_bot', ierr); call CHKERR(ierr)

      call DMCreateFieldIS(solver%mergedm TODO: get IS for merged edir edn eup

      ! Concatenate merged and 3D domain(without top)
      call ISDifference( &
        & solver%fields(FIELD_EDIR_3D_TOP)%subIS,     &
        & solver%fields(FIELD_EDIR_3D_TOP)%topfaceIS, &
        & istmp, ierr); call CHKERR(ierr)

      !call ISExpand(isedir, istmp, isedir, ierr); call CHKERR(ierr)
      call ISSort(isedir, ierr); call CHKERR(ierr)

      call ISExpand(solver%fields(FIELD_EDIF_COLLAPSE)%subIS, solver%fields(FIELD_EDIF_1D)%subIS, isediff, ierr); call CHKERR(ierr)
      call ISExpand(isediff, solver%fields(FIELD_EDIF_3D_TOP)%subIS, isediff, ierr); call CHKERR(ierr)

      associate(solution => solver%solutions(uid))
        call VecGetSubVector(solution%flx, solver%fields(FIELD_EDIR_COLLAPSE)%subIS, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_collapse_edir', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, solver%fields(FIELD_EDIR_COLLAPSE)%subIS, subvec, ierr); call CHKERR(ierr)

        call VecGetSubVector(solution%flx, solver%fields(FIELD_EDIR_1D)%subIS, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_1D_edir', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, solver%fields(FIELD_EDIR_1D)%subIS, subvec, ierr); call CHKERR(ierr)

        call VecGetSubVector(solution%flx, solver%fields(FIELD_EDIR_3D_TOP)%subIS, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_3D_edir', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, solver%fields(FIELD_EDIR_3D_TOP)%subIS, subvec, ierr); call CHKERR(ierr)

        call VecGetSubVector(solution%flx, solver%fields(FIELD_EDIF_COLLAPSE)%subIS, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_collapse_ediff', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, solver%fields(FIELD_EDIF_COLLAPSE)%subIS, subvec, ierr); call CHKERR(ierr)

        call VecGetSubVector(solution%flx, solver%fields(FIELD_EDIF_1D)%subIS, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_1D_ediff', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, solver%fields(FIELD_EDIF_1D)%subIS, subvec, ierr); call CHKERR(ierr)

        call VecGetSubVector(solution%flx, solver%fields(FIELD_EDIF_3D_TOP)%subIS, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_3D_ediff', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, solver%fields(FIELD_EDIF_3D_TOP)%subIS, subvec, ierr); call CHKERR(ierr)

        call VecGetSubVector(solution%flx, isedir, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_edir', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, isedir, subvec, ierr); call CHKERR(ierr)

        call VecGetSubVector(solution%flx, isediff, subvec, ierr); call CHKERR(ierr)
        call PetscObjectViewFromOptions(subvec, PETSC_NULL_VEC, '-show_ediff', ierr); call CHKERR(ierr)
        call VecRestoreSubVector(solution%flx, isediff, subvec, ierr); call CHKERR(ierr)
      end associate


      print *,'DEBUG',solver%plex%comm,redn,reup,rabso,redir,opt_solution_uid
