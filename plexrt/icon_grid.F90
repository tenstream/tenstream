...
contains
  ...
  subroutine bcast_icongrid(comm, icongrid)
    ...
    call imp_bcast(comm, icongrid%Nfaces, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%Nedges, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%Nvertices, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%cell_index, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%edge_index, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%vertex_index, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%cartesian_x_vertices, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%cartesian_y_vertices, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%cartesian_z_vertices, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%vertex_of_cell, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%edge_of_cell, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%edge_vertices, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%adj_cell_of_edge, 0_mpiint, ierr); call CHKERR(ierr)
    call imp_bcast(comm, icongrid%cells_of_vertex, 0_mpiint, ierr); call CHKERR(ierr)
  end subroutine
  ...
  subroutine decompose_icon_grid_parmetis(comm, icongrid, cellowner, edgeowner, vertexowner, cell_ao)
    ...
    call ISDestroy(isg, ierr); call CHKERR(ierr)
    call ISDestroy(is_my_icon_cells, ierr); call CHKERR(ierr)
    call MatPartitioningDestroy(part, ierr); call CHKERR(ierr)
    call MatDestroy(mesh, ierr); call CHKERR(ierr)
    call MatDestroy(dual, ierr); call CHKERR(ierr)
