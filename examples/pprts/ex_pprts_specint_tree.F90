...
#include "petsc/finclude/petsc.h"
  ...
  lsolar = .true.
  call get_petsc_opt('', '-solar', &
                     lsolar, lflg, ierr); call CHKERR(ierr)

  lthermal = .true.
  call get_petsc_opt('', '-thermal', &
  ...
  Ntree_height = 4
  call get_petsc_opt('', "-Ntree", Ntree_height, lflg, ierr); call CHKERR(ierr)

  luse_usgs_db = .false.
  call get_petsc_opt('', '-usgs', &
