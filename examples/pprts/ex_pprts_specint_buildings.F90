...
!  if(.not.lflg) call CHKERR(1_mpiint, 'need to supply a output filename... please call with -out <output.nc>')
  ...
  lbuildings = .true.
  call get_petsc_opt('', '-buildings', &
                     lbuildings, lflg, ierr); call CHKERR(ierr)

  buildings_albedo = .1_ireals
  call get_petsc_opt('', "-BAg", buildings_albedo, lflg, ierr); call CHKERR(ierr)

  buildings_temp = 300
  call get_petsc_opt('', "-Btemp", buildings_temp, lflg, ierr); call CHKERR(ierr)
