...
#include "petsc/finclude/petsc.h"
  ...
  albedo_th = 0._ireals
  call get_petsc_opt('', "-Ag", albedo_th, lflg, ierr); call CHKERR(ierr)
  call get_petsc_opt('', "-Ag_thermal", albedo_th, lflg, ierr); call CHKERR(ierr)
  albedo_sol = .1_ireals
  call get_petsc_opt('', "-Ag", albedo_sol, lflg, ierr); call CHKERR(ierr)
  call get_petsc_opt('', "-Ag_solar", albedo_sol, lflg, ierr); call CHKERR(ierr)
  ...
  vlwc = 0
  call get_petsc_opt('', "-lwc", vlwc, lflg, ierr); call CHKERR(ierr)

  viwc = 0
  call get_petsc_opt('', "-iwc", viwc, lflg, ierr); call CHKERR(ierr)

  vreff = 10
  call get_petsc_opt('', "-reff", vreff, lflg, ierr); call CHKERR(ierr)
