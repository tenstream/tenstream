#!/bin/bash
module purge
module load petsc/double-gcc-9
...
basename = '_'.join(args.inp.split('_')[0:3])
D = xr.open_dataset(args.inp)
D3 = xr.open_dataset('{}_rayli_ac.nc'.format(basename))
...
E = lambda D: D['edir'].data[:].mean(axis=1).flatten()
plt.title('rrmse = {:.2f}, bias = {:.2f}'.format(rrmse(E(D),E(D3)), bias(E(D),E(D3))))
plt.pcolormesh(y,z, D['edir'].mean(axis=1))
cbar = plt.colorbar(); cbar.set_label('edir [W/m^2]'); plt.ylim(*yrng)

plt.subplot(412)
E = lambda D: D['edn'].data[:].mean(axis=1).flatten()
plt.title('rrmse = {:.2f}, bias = {:.2f}'.format(rrmse(E(D),E(D3)), bias(E(D),E(D3))))
plt.pcolormesh(y,z, D['edn'].mean(axis=1))
cbar = plt.colorbar(); cbar.set_label('edn [W/m^2]'); plt.ylim(*yrng)

plt.subplot(413)
E = lambda D: D['eup'].data[:].mean(axis=1).flatten()
plt.title('rrmse = {:.2f}, bias = {:.2f}'.format(rrmse(E(D),E(D3)), bias(E(D),E(D3))))
...
plt.subplot(414)
E = lambda D: D['abso'].data[:].mean(axis=1).flatten()
plt.title('rrmse = {:.2f}, bias = {:.2f}'.format(rrmse(E(D),E(D3)), bias(E(D),E(D3))))
...
function runex {
  ...
  if [ -e $OUT ]; then
    ...
    echo "Running $OUT"
    ($EXEC bin/$bin $baseopt -out $OUT $OPT | tee log.$OUT) &
  fi
}

mpiexec="srun --time=08:00:00 -n 40 -N 1-4 -C avx -p met-ws,cluster --mpi=pmix"
rayexec="srun --time=08:00:00 -n 2 -N 2 -c 10 -C avx -p met-ws,cluster --mpi=pmix"

for phi in 180,195,210,225
do
for SZA in 0 20 40
#for SZA in 40
do
  #runex "$mpiexec" "res_${phi}_${SZA}_1d.nc                  "  "-theta0 $SZA  -twostr_only"
  runex "$rayexec" "res_${phi}_${SZA}_rayli_ac.nc            "  "-theta0 $SZA -phi0 $phi -pprts_atm_correction $rayli_opt"
  runex "$mpiexec" "res_${phi}_${SZA}_3_10str.nc             "  "-theta0 $SZA -phi0 $phi"
  runex "$mpiexec" "res_${phi}_${SZA}_3_10str_ac.nc             "  "-theta0 $SZA -phi0 $phi -pprts_atm_correction "
  #runex "$mpiexec" "res_${phi}_${SZA}_3_10str_distorted_ac.nc"  "-theta0 $SZA  -pprts_atm_correction -bmc_online"
  runex "$mpiexec" "res_${phi}_${SZA}_3_10str_gomtrc_ac.nc   "  "-theta0 $SZA -phi0 $phi -pprts_atm_correction -pprts_geometric_coeffs"
  wait

  [ ! -e plot_srfc_${phi}_${SZA}.pdf ] && python plot_distorted.py res_${phi}_${SZA}_ --out plot_srfc_${phi}_${SZA}.pdf
  for f in res_${phi}_${SZA}_*.nc; do
