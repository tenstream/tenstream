...
module m_f2c_rayli
  ...
  interface
    integer(c_int) function rfft_wedgeF90(&
        ...
        kabs, ksca, g, sundir, &
        flx_through_faces) bind(c, name='rfft_wedge')
      use iso_c_binding
      ...
      real(c_double) :: flx_through_faces(1:Nfaces)
    end function
  end interface
...
contains
  integer(c_int) function rfft_wedgeF90(&
      ...
      real(c_double), intent(out) :: flx_through_faces(:)
      ...
      if(.False.) then ! unused var warnings
        flx_through_faces(1) = real(Nphotons+Nwedges+Nfaces+Nverts+verts_of_face(1,1)+wedges_of_face(1,1), c_double) + &
          vert_coords(1,1) + kabs(1) + ksca(1) + g(1) + sundir(1)
      endif
    end function
