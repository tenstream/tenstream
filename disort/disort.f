...
c FORTRAN 77
c
c Some minor modifications have been done to fit the needs in the Tenstream lib
c Major thing is the intro of Plank fractions to use it with RRTMG
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
c     Added this variable to account for the Planck fractions noted
c     in RRTM
      real :: Bfracs(maxcly)
...
c    In order to pass the self-tests, let PASS1 go without including
c    effect of Planck fraction
        if(pass1) then
...
c    effect of Planck fraction
            ...
            XR1( LC ) = 0.0
caer With the correlated-k method employed in RRTM the upper regions of
c    the atmospheres with small optical depths at particular g-points.
c    In the original code in DISORT (now commented out), the DTAUCP
c    is very small and produces erroneous emission for layers.  AER
c    has added a test to determine when the original equation does not
c    work suitably.  In the cases with small optical depth, we have just
c    duplicated the original code for when DTAUCP equals zero.

c            IF( DTAUCP( LC ).GT.0.0 ) THEN
c              XR1( LC ) = ( PKAG( LC ) - PKAG( LC-1 ) ) / DTAUCP( LC )
c            ENDIF
c
c            XR0( LC ) = PKAG( LC-1 ) - XR1( LC )*TAUCPR( LC-1 )
