...
MPIEXEC=${MPIEXEC:-}
if [ ! -e ediff.h5 ]; then
$MPIEXEC bin/ex_pprts_box_cld -pprts_show_edir hdf5:edir.h5 -pprts_show_ediff hdf5:ediff.h5 -pprts_show_abso hdf5:abso.h5 -solver 3_10 -pprts_solver_view -Xpprts_open_bc $@
fi

echo ipython --pylab -c \
  'import h5py as H; \
  ediff = H.File("ediff.h5","r")["ediff0"][:]; \
  abso = H.File("abso.h5","r")["abso0"][:]; \
  Nx,Ny = int(ediff.shape[1]/2-.5), int(ediff.shape[0]/2-.5); \
  print(Nx, Ny);\
  figure(figsize=(14,8));\
