        ...
        if (.not. lexit_cell) then
          ...
          case (3)
            ...
            if (p%i .eq. solver%C_one%xs - 1) then
              if (ldebug) &
                print *, myid, cstr('********************* Sending to WEST', 'blue'), pqueues(PQ_WEST)%owner, p%k, p%i, p%j
          ...
          case (4)
            ...
            if (p%i .eq. solver%C_one%xe + 1) then
              if (ldebug) &
                print *, myid, cstr('********************* Sending to EAST', 'blue'), pqueues(PQ_EAST)%owner, p%k, p%i, p%j
          ...
          case (5)
            ...
            if (p%j .eq. solver%C_one%ys - 1) then
              if (ldebug) &
                print *, myid, cstr('********************* Sending to SOUTH', 'blue'), pqueues(PQ_SOUTH)%owner, p%k, p%i, p%j
          ...
          case (6)
            ...
            if (p%j .eq. solver%C_one%ye + 1) then
              if (ldebug) &
                print *, myid, cstr('********************* Sending to NORTH', 'blue'), pqueues(PQ_NORTH)%owner, p%k, p%i, p%j
  ...
  subroutine update_flx(p, k, i, j, xv_dir, xv_diff)
    ...
    case (1)
      if (k .lt. lbound(xv_diff, 2) .or. k .gt. ubound(xv_diff, 2)) &
        call CHKERR(1_mpiint, 'invalid k '//itoa(k)//' '// &
