...
contains
    ...
    if (ldebug) then
      do k = 1, ke1
        if (any(isnan([Eup(k), Edn(k)]))) &
          print *, 'setting value for Eup,Edn', k, &
          ' LAPACK entries', B(2 * k - 1, 1), B(2 * k, 1), &
          ' Eup/dn', Eup(k), Edn(k), &
          ' IPIV', IPIV(2 * k - 1:2 * k)
      end do
  ...
  subroutine adding_delta_eddington_twostream(dtau, omega0, g, mu0, S0, Ag, Edir, Edn, Eup)
    ...
    do k = 1, ke - 1
      ...
      Sdir(k + 1) = (a11(k + 1) * Sdir(k) &
                     + Tdir(k) * a13(k + 1) * R(k) * a11(k + 1)) / (one - R(k) * a12(k + 1)) &
                    + Tdir(k) * a23(k + 1)
    end do
    ...
    do k = ke1, 3, -1
      Edn(k - 1) = (R(k - 2) * a11(k - 1) * Eup(k) &
                    + Edir(1) * Sdir(k - 2) &
                    + Edir(k - 1) * a13(k - 1) * R(k - 2)) / (one - R(k - 2) * a12(k - 1))
      Eup(k - 1) = (a11(k - 1) * Eup(k) &
                    + Edir(1) * Sdir(k - 2) * a12(k - 1) &
                    + Edir(k - 1) * a13(k - 1)) / (one - R(k - 2) * a12(k - 1))
    end do
