          ...
          if (solver%atm%l1d(atmk(solver%atm, k))) then
            ...
            do idst = 1, C%dof
              ...
              if (C%neighbors(10) .ne. myid &
                  ...
                  .and. row(MatStencil_j, idst) .lt. C%xs) llocal_dst = .false. ! have real neighbor west  and is not local entry
              if (C%neighbors(16) .ne. myid &
                  .and. C%neighbors(16) .ge. i0 &
                  .and. row(MatStencil_j, idst) .gt. C%xe) llocal_dst = .false. ! have real neighbor east  and is not local entry
              if (C%neighbors(4) .ne. myid &
                  .and. C%neighbors(4) .ge. i0 &
                  .and. row(MatStencil_k, idst) .lt. C%ys) llocal_dst = .false. ! have real neighbor south and is not local entry
              if (C%neighbors(22) .ne. myid &
                  .and. C%neighbors(22) .ge. i0 &
                  .and. row(MatStencil_k, idst) .gt. C%ye) llocal_dst = .false. ! have real neighbor north and is not local entry
              ...
              do isrc = 1, C%dof
                ...
                if (C%neighbors(10) .ne. myid &
                    .and. C%neighbors(10) .ge. i0 &
                    .and. col(MatStencil_j, isrc) .lt. C%xs) llocal_src = .false. ! have real neighbor west  and is not local entry
                if (C%neighbors(16) .ne. myid &
                    .and. C%neighbors(16) .ge. i0 &
                    .and. col(MatStencil_j, isrc) .gt. C%xe) llocal_src = .false. ! have real neighbor east  and is not local entry
                if (C%neighbors(4) .ne. myid &
                    .and. C%neighbors(4) .ge. i0 &
                    .and. col(MatStencil_k, isrc) .lt. C%ys) llocal_src = .false. ! have real neighbor south and is not local entry
                if (C%neighbors(22) .ne. myid &
                    .and. C%neighbors(22) .ge. i0 &
                    .and. col(MatStencil_k, isrc) .gt. C%ye) llocal_src = .false. ! have real neighbor north and is not local entry
        ...
        do k = C%zs, C%ze - 1
          ...
          if (solver%atm%l1d(atmk(solver%atm, k))) then
            ...
            do idst = 0, C%dof - 1
              ...
              if (C%neighbors(10) .ne. myid &
                  .and. C%neighbors(10) .ge. i0 &
                  .and. row(MatStencil_j, idst) .lt. C%xs) dst_id = C%neighbors(10) ! have real neighbor west  and is not local entry
              if (C%neighbors(16) .ne. myid &
                  .and. C%neighbors(16) .ge. i0 &
                  .and. row(MatStencil_j, idst) .gt. C%xe) dst_id = C%neighbors(16) ! have real neighbor east  and is not local entry
              if (C%neighbors(4) .ne. myid &
                  .and. C%neighbors(4) .ge. i0 &
                  .and. row(MatStencil_k, idst) .lt. C%ys) dst_id = C%neighbors(4) ! have real neighbor south and is not local entry
              if (C%neighbors(22) .ne. myid &
                  .and. C%neighbors(22) .ge. i0 &
                  .and. row(MatStencil_k, idst) .gt. C%ye) dst_id = C%neighbors(22) ! have real neighbor north and is not local entry
              ...
              do isrc = 0, C%dof - 1
                ...
                if (C%neighbors(10) .ne. myid &
                    .and. C%neighbors(10) .ge. i0 &
                    .and. col(MatStencil_j, isrc) .lt. C%xs) src_id = C%neighbors(10) ! have real neighbor west  and is not local entry
                if (C%neighbors(16) .ne. myid &
                    .and. C%neighbors(16) .ge. i0 &
                    .and. col(MatStencil_j, isrc) .gt. C%xe) src_id = C%neighbors(16) ! have real neighbor east  and is not local entry
                if (C%neighbors(4) .ne. myid &
                    .and. C%neighbors(4) .ge. i0 &
                    .and. col(MatStencil_k, isrc) .lt. C%ys) src_id = C%neighbors(4) ! have real neighbor south and is not local entry
                if (C%neighbors(22) .ne. myid &
                    .and. C%neighbors(22) .ge. i0 &
                    .and. col(MatStencil_k, isrc) .gt. C%ye) src_id = C%neighbors(22) ! have real neighbor north and is not local entry
  ...
  subroutine set_optical_properties(solver, albedo, &
    ...
    associate (atm => solver%atm, &
      ...
      if (luse_eddington) then
        if (.not. allocated(atm%a11)) &
          allocate (atm%a11(C_one_atm%zs:C_one_atm%ze, C_one_atm%xs:C_one_atm%xe, C_one_atm%ys:C_one_atm%ye))   ! allocate space for twostream coefficients
        if (.not. allocated(atm%a12)) &
          allocate (atm%a12(C_one_atm%zs:C_one_atm%ze, C_one_atm%xs:C_one_atm%xe, C_one_atm%ys:C_one_atm%ye))
        if (.not. allocated(atm%a21)) &
          allocate (atm%a21(C_one_atm%zs:C_one_atm%ze, C_one_atm%xs:C_one_atm%xe, C_one_atm%ys:C_one_atm%ye))
        if (.not. allocated(atm%a22)) &
          allocate (atm%a22(C_one_atm%zs:C_one_atm%ze, C_one_atm%xs:C_one_atm%xe, C_one_atm%ys:C_one_atm%ye))
        if (.not. allocated(atm%a13)) &
          allocate (atm%a13(C_one_atm%zs:C_one_atm%ze, C_one_atm%xs:C_one_atm%xe, C_one_atm%ys:C_one_atm%ye))
        if (.not. allocated(atm%a23)) &
          allocate (atm%a23(C_one_atm%zs:C_one_atm%ze, C_one_atm%xs:C_one_atm%xe, C_one_atm%ys:C_one_atm%ye))
        if (.not. allocated(atm%a33)) &
          allocate (atm%a33(C_one_atm%zs:C_one_atm%ze, C_one_atm%xs:C_one_atm%xe, C_one_atm%ys:C_one_atm%ye))
      end if
    ...
    subroutine check_arr_size(C, inp)
      ...
      if (allocated(inp)) then
        if (size(inp) .ne. C%dof * C%glob_zm * C%glob_xm * C%glob_ym) then
          ...
          print *, 'Size of global Dimensions of simulation:', C%dof * C%glob_zm * C%glob_xm * C%glob_ym, &
