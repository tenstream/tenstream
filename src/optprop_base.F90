  ...
  subroutine set_op_param_space(OPP, cname, ierr)
    ...
    if (.not. lLUT_mockup) then
      ...
      case ('LUT_3_10_for_ANN')
        ...
        call populate_op_dim('tanx', 11_iintegers, OPP%dirconfig%dims(7), vrange=real([-.5, .5], irealLUT))
        call populate_op_dim('tany', 11_iintegers, OPP%dirconfig%dims(8), vrange=real([-.5, .5], irealLUT))
        allocate (OPP%diffconfig%dims(6))
        call populate_op_dim('tau', OPP%diffconfig%dims(1), preset=preset_tau31)
        call populate_op_dim('w0', OPP%diffconfig%dims(2), preset=preset_w020)
        call populate_op_dim('aspect_zx', OPP%diffconfig%dims(3), preset=preset_aspect23)
        call populate_op_dim('g', OPP%diffconfig%dims(4), preset=preset_g6)
        call populate_op_dim('tanx', 11_iintegers, OPP%diffconfig%dims(5), vrange=real([-.5, .5], irealLUT))
        call populate_op_dim('tany', 11_iintegers, OPP%diffconfig%dims(6), vrange=real([-.5, .5], irealLUT))
      ...
      case ('LUT_3_10_for_ANN')
        ...
        call populate_op_dim('tanx', 3_iintegers, OPP%dirconfig%dims(7), vrange=real([-.5, .5], irealLUT))
        call populate_op_dim('tany', 3_iintegers, OPP%dirconfig%dims(8), vrange=real([-.5, .5], irealLUT))
        allocate (OPP%diffconfig%dims(6))
        ...
        call populate_op_dim('tanx', 3_iintegers, OPP%diffconfig%dims(5), vrange=real([-.5, .5], irealLUT))
        call populate_op_dim('tany', 3_iintegers, OPP%diffconfig%dims(6), vrange=real([-.5, .5], irealLUT))
      case ('LUT_3_16')
        call setup_pprts_mockup()
      case ('LUT_3_24')
        call setup_pprts_mockup()
