module boxmc_oop
      ...
      abstract interface
        subroutine update_diff_stream(bmc,p,S,N)
          import :: boxmc,photon,iintegers,ireals
          class(boxmc) :: bmc
...
subroutine get_coeff(bmc,comm,op_bg,src,S_out,Sdir_out,dir,deltascale,phi0,theta0,dx,dy,dz)
        class(boxmc) :: bmc
        ...
        double precision,intent(in) :: dx,dy,dz
        double precision,intent(out):: S_out(bmc%diff_streams),Sdir_out(bmc%dir_streams)
        ...
        do k=1,mycnt
                ...
                if(dir) then
                        call bmc%init_dir_photon(p,src,dir,initial_dir,dx,dy,dz)
                else
                ...
                if(dir) call std_update( std_Sdir , k, i1*numnodes )
        ...
        if(dir) Sdir_out = dble( std_Sdir%mean *k )
        if(myid.ge.0) then
