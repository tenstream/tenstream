...
contains
  ...
  subroutine explicit_edir(solver, prefix, edirTOA, solution, ierr)
    ...
    type(tVec) :: v0, b, lb
#else
    real(ireals), allocatable, target :: v0(:, :, :, :), b(:, :, :, :), lb(:, :, :, :)
#endif
    ...
    associate ( &
...
#else
      allocate (b(0:C%dof - 1, C%zs:C%ze, C%xs:C%xe, C%ys:C%ye)); b = zero
      call setup_incSolar(solver, edirTOA, b)
      allocate (lb(0:C%dof - 1, C%zs:C%ze, C%gxs:C%gxe, C%gys:C%gye)); lb = zero
      lb(:, :, C%xs:C%xe, C%ys:C%ye) = b
      allocate (v0(0:C%dof - 1, C%zs:C%ze, C%gxs:C%gxe, C%gys:C%gye)); v0 = zero
      v0(:, :, C%xs:C%xe, C%ys:C%ye) = vedir
#endif
      ...
      call PetscObjectViewFromOptions(PetscObjectCast(vedir), PETSC_NULL_OBJECT, "-show_debug_edir", ierr)
      call CHKERR(ierr)
#else
      nullify (xg, x0)
      deallocate (v0, lb, b)
      ...
      subroutine explicit_ediff(solver, prefix, vb, solution, ierr)
        ...
        subroutine explicit_ediff(solver, prefix, vb, solution, ierr)
          ...
          associate ( &
            ...
            call PetscObjectViewFromOptions(PetscObjectCast(vediff), PETSC_NULL_OBJECT, "-show_debug_ediff", ierr)
            call CHKERR(ierr)
#else
            nullify (xg, x0)
            deallocate (v0, lvb)
