...
private
public :: optprop_1_2_init, optprop_1_2_lookup_coeff,optprop_1_2_debug
integer :: ierr

logical :: optprop_1_2_debug=.False.
...
contains
  ...
  subroutine optprop_1_2_lookup_coeff(dz,kabs,ksca,g,dir,C,inp_angles)
...
! This enables on-line calculations of coefficients with bmc code. This takes FOREVER! - use this only to check if LUT is working correctly!
        if(determine_coeff_error) then 
                ...
                if(optprop_1_2_debug.and.(T_dir(1).le.checking_limit)) then
                        if(present(inp_angles)) then 
                                if(dir) then !dir2dir
