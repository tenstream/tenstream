module tenstream_optprop_LUT_1_2
  ...
  private
  public :: init_LUT,LUT_get_dir2dir,LUT_get_dir2diff,LUT_get_diff2diff,bmc_wrapper
...
contains
  subroutine scatter_LUTtables(azis,szas,comm)
      ...
      call determine_angles_to_load(dirLUT, azis, szas, angle_mask)
...
subroutine loadLUT_dir(LUT, azis,szas,comm)
    ...
    call determine_angles_to_load(LUT, azis, szas, angle_mask)
...
subroutine createLUT_diff(LUT,coeff_table_name,comm)
    ...
    do ig    =1,Ng
      ...
      enddo !ksca
    enddo !g
    if(myid.eq.0) print *,'done calculating diffuse coefficients',shape(LUT%S%c),'min/max',minval(LUT%S%c),maxval(LUT%S%c)
end subroutine
